...
For Option 3C hybrid mode, OpenWebUI is configured for plain email/password auth (`WEBUI_AUTH=true`, no trusted headers). demo-site performs the `/api/openwebui/start` bootstrap by validating `demo_session`, pre-creating the reviewer user when admin credentials are configured, signing the reviewer in with a deterministic password derived from the demo session token and `DEMO_SITE_OPENWEBUI_PASSWORD_SECRET` (defaults to `WEBUI_SECRET_KEY`), and relaying OpenWebUI `Set-Cookie` headers to the browser. `/api/openwebui/start` fails closed with `503 openwebui_password_secret_missing` if neither secret is set.
...
| D27 | OpenWebUI trusted-header SSO (`WEBUI_AUTH_TRUSTED_*_HEADER`, `X-Demo-Email`/`X-Demo-Name`) | Replaced with email/password bootstrap: `WEBUI_AUTH=true` only; demo-site derives a per-reviewer password from `HMAC-SHA256(DEMO_SITE_OPENWEBUI_PASSWORD_SECRET, "reviewer:" + session.token)` and signs the reviewer in with `{email, password}`. Reviewer namespace moved from `demo-<prefix>@mmrag.invalid` to `demo2-<prefix>@mmrag.invalid` to avoid collisions with legacy trusted-header users. `/api/openwebui/start` fails closed with `503 openwebui_password_secret_missing` when no secret is configured (default falls back to `WEBUI_SECRET_KEY`). | Unblocks direct internal OpenWebUI login at the internal Serve slot for Ammer/Sven and removes the trusted-header attack surface. The proxy no longer injects `X-Demo-Email`/`X-Demo-Name`; incoming spoofs are still stripped. Old `demo-` users remain in OpenWebUI as harmless orphans. |
